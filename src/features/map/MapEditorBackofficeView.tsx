...
export const MapEditorBackofficeView: React.FC = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        if (event.target?.result) setCustomMapImage(event.target.result as string);
      };
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-5 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="bg-amber-500 text-black px-2 py-0.5 rounded text-[10px] font-black uppercase">Back Office</span>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="btn-pill btn-pill-inactive flex items-center gap-1.5" title="Upload custom map JPG/PNG">
            <Upload className="w-3.5 h-3.5 text-cyan-400" />
            <span>Upload Map Image</span>
          </button>
          {customMapImage && (
            <button onClick={() => setCustomMapImage(null)} className="btn-pill btn-pill-inactive text-neutral-300" title="Reset to default Coral Island game map">
              Default Map
            </button>
          )}
          <button onClick={() => { setJsonText(JSON.stringify(customLocations, null, 2)); setJsonModalOpen(true); }} className="btn-pill btn-pill-inactive flex items-center gap-1.5">
            <Download className="w-3.5 h-3.5 text-emerald-400" />
            <span>Export JSON</span>
          </button>
          <button
            onClick={() => { if (window.confirm('Reset all map pins and locations back to factory defaults?')) resetLocationsToDefault(); }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-2 glass-panel p-4 sm:p-5 shadow-xl space-y-3">
          ...
          >
            <img src={mapImageSrc} alt="Back Office Official Map Canvas" className="w-full h-full object-contain rounded-2xl pointer-events-none" />

            {customLocations.map(loc => {
              const isActive = loc.id === activeLocation.id;
              const allSpots: MapSpotCoordinate[] = loc.spots && loc.spots.length > 0 ? loc.spots : [{ id: 'primary', x: loc.x, y: loc.y, label: 'Primary Spot' }];
              const colorClass = loc.category === 'Ocean' ? 'bg-blue-500' : loc.category === 'Cave' ? 'bg-purple-600' : loc.category === 'Special' ? 'bg-orange-500' : 'bg-emerald-500';
              ...
              return allSpots.map((spot, idx) => (
                <div
                  ...
                  className={`absolute transform -translate-x-1/2 -translate-y-1/2 p-1.5 rounded-full shadow-lg transition-transform pointer-events-none ${
                    isActive ? 'bg-amber-400 text-black scale-125 ring-4 ring-amber-300 z-30' : `${colorClass} text-white opacity-70 z-10`
                  }`}
        ...
        <div className="glass-panel p-5 shadow-xl space-y-4 text-xs">
          <div className="space-y-1.5">
            <span className="text-[10px] uppercase font-bold tracking-wider text-neutral-400 block">1. Select Water Body to Edit</span>
            <select
            ...
            >
              {customLocations.map(loc => (
                <option key={loc.id} value={loc.id}>{loc.name} ({loc.category}) - {(loc.spots?.length || 1)} spot(s)</option>
              ))}
            </select>
          </div>

          <div className="space-y-1.5">
            <span className="text-[10px] uppercase font-bold tracking-wider text-neutral-400 block">2. Optional Custom Label for Next Click</span>
            <input
              type="text"
              value={newSpotLabel}
          ...
          <div className="border-t border-white/10 pt-3 space-y-2">
            ...
            <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1">
              {(activeLocation.spots || []).map((spot, index) => (
                <div key={spot.id} className="bg-black/40 p-2.5 rounded-xl border border-white/5 flex items-center justify-between gap-2 text-neutral-200">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <strong className="block font-bold text-white text-[11px]">{spot.label || `Spot #${index + 1}`}</strong>
                      <span className="text-[10px] text-neutral-400 font-mono">X: {spot.x}%, Y: {spot.y}%</span>
                    </div>
                  </div>
                  <button
              ...
              {(!activeLocation.spots || activeLocation.spots.length === 0) && (
                <p className="text-neutral-500 italic py-4 text-center">No spots configured yet. Click on the map to place a pin!</p>
              )}
            </div>
          </div>
        </div>
      ...
      {jsonModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="cg-card p-6 rounded-2xl max-w-xl w-full space-y-4 border border-amber-400/40">
            ...
            <textarea readOnly value={jsonText} rows={12} className="w-full bg-[#ede5d5] p-3 rounded-xl font-mono text-xs text-[#3d2f1a] border border-[#d1c2ab] focus:outline-none select-all" />
            <div className="flex justify-end gap-2">
              <button onClick={() => setJsonModalOpen(false)} className="btn-amber text-xs py-1.5 px-4">Close</button>
            </div>
          </div>
        </div>
      )}
