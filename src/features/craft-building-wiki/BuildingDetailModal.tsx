    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-white leading-tight">{activeTier.name}</h2>
                <span className="bg-amber-500/20 text-amber-300 border border-amber-400/40 text-[10px] font-black px-2 py-0.5 rounded-full">
                  {building.category}
                </span>
              </div>
              <span className="text-xs text-[#c4b5a0]">Constructed by {building.builder}</span>
            </div>
          </div>
        ...
        {building.tiers.length > 1 && (
          <div className="flex items-center gap-2 bg-[#13181b] p-1.5 rounded-2xl border border-white/10">
            {building.tiers.map((tier, idx) => (
              ...
              >
                Level {tier.tierNumber}: {tier.name}
              </button>
            ))}
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          ...
          <div className="bg-[#13181b] p-3 rounded-2xl border border-white/10 space-y-0.5 text-center">
            <span className="text-[10px] text-[#c4b5a0] uppercase font-bold flex items-center justify-center gap-1">
              <Clock className="w-3.5 h-3.5 text-cyan-400" /> Build Time
            </span>
            <span className="text-sm font-black text-white">
          ...
          <div className="bg-[#13181b] p-3 rounded-2xl border border-white/10 space-y-0.5 text-center">
            <span className="text-[10px] text-[#c4b5a0] uppercase font-bold flex items-center justify-center gap-1">
              <Maximize2 className="w-3.5 h-3.5 text-purple-400" /> Size
            </span>
        ...
        <div className="bg-amber-950/30 border border-amber-500/30 p-4 rounded-2xl space-y-2.5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-amber-300 uppercase tracking-wider">
              {activeTier.capacityText ? `Capacity: ${activeTier.capacityText}` : 'Building Features'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {activeTier.materials.map((mat, idx) => (
              ...
              >
                ...
                <span className="font-black text-amber-300 text-xs">{mat.amount}x</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full py-3 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 text-black hover:from-amber-400 hover:to-orange-400 transition-all shadow-lg shadow-amber-500/25"
          >
            {isPlanned ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
            <span>Add {activeTier.name} to Farm Project Planner</span>
          </button>
        </div>
      </div>
    </div>
