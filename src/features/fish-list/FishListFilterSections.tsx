...
export const DateWeatherFilterSection: React.FC<DateWeatherProps> = ({
  ...
  const timeSlots: Array<{ id: TimeOfDay; label: string; icon: string }> = [
    { id: 'morning', label: t('time_morning'), icon: '🌅' },
    { id: 'afternoon', label: t('time_afternoon'), icon: '☀️' },
    { id: 'evening', label: t('time_evening'), icon: '🌇' },
    { id: 'night', label: t('time_night'), icon: '🌙' }
  ];
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {seasons.map(s => (
            ...
            >
              ...
              <span className="text-[9px] truncate max-w-[50px]">{s.label}</span>
            </button>
          ))}
        </div>
          ...
          {timeSlots.map(tSlot => (
            <button
              ...
              className={`cg-pill py-1.5 px-2 text-[11px] ${
                timeOfDay === tSlot.id ? 'cg-pill-active' : ''
              }`}
            >
              <span>{tSlot.icon}</span>
              <span>{tSlot.label}</span>
            </button>
          ))}
        </div>
    ...
    <div className="border-t border-white/10 pt-3 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {Object.values(RODS_DATA).map(rod => (
          ...
          >
            {t(`rod_${rod.id}` as any, rod.name.split(' ')[0])}
          </button>
        ))}
      </div>
    </div>
