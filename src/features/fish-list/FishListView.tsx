...
import { FishListSidebar } from './FishListSidebar';
...
export const FishListView: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
      ...
      {/* Left Column: Side Panel with All Selections & Controls */}
      <div className="lg:col-span-1 lg:sticky lg:top-20">
        <FishListSidebar />
      </div>

      {/* Right Column: Main Fish Catalog List */}
      <div className="lg:col-span-3 space-y-4">
        
        {/* Top Control Bar: Active Match Count & Sorting */}
        <div className="glass-panel p-3 sm:px-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 text-xs">
          
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-bold text-white text-sm">
              Fish Catalog ({filteredFish.length})
            </span>
            <span className="text-[#c4b5a0] font-medium">
              • {gameState.season.toUpperCase()}, Day {gameState.day} ({gameState.timeOfDay})
            </span>
          ...
          <div className="flex items-center gap-1.5 w-full sm:w-auto justify-end">
            ...
            >
              <option value="priority-desc" className="bg-[#13181b] text-white">🚩 Priority: Exclusives & Needed First</option>
        ...
        {filteredFish.length > 0 ? (
          ...
          <div className="cg-card p-10 text-center space-y-3">
            ...
            <p className="text-xs text-[#8c785b] max-w-sm mx-auto">
              No fish match your current side panel filters. Try adjusting season, time, weather, or resetting filters.
            </p>
            <button
              onClick={() =>
                setFilters(prev => ({
